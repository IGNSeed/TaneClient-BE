...
std::atomic<float> g_lineColorR = 1.0f;
std::atomic<float> g_lineColorG = 1.0f;
std::atomic<float> g_lineColorB = 1.0f;
std::atomic<float> g_lineColorA = 0.78f;

bool BuildTracerConfigPath(wchar_t* path, DWORD pathCount) {
    ...
    return swprintf_s(path, pathCount, L"%s\\Tracer.json", renderPath) >= 0;
}
...
Color LoadTracerColor() {
    return Color{
        std::clamp(g_lineColorR.load(std::memory_order_acquire), 0.0f, 1.0f),
        std::clamp(g_lineColorG.load(std::memory_order_acquire), 0.0f, 1.0f),
        std::clamp(g_lineColorB.load(std::memory_order_acquire), 0.0f, 1.0f),
        std::clamp(g_lineColorA.load(std::memory_order_acquire), 0.0f, 1.0f),
    };
}

void StoreTracerColor(const Color& color) {
    g_lineColorR.store(std::clamp(color.r, 0.0f, 1.0f), std::memory_order_release);
    g_lineColorG.store(std::clamp(color.g, 0.0f, 1.0f), std::memory_order_release);
    g_lineColorB.store(std::clamp(color.b, 0.0f, 1.0f), std::memory_order_release);
    g_lineColorA.store(std::clamp(color.a, 0.0f, 1.0f), std::memory_order_release);
}
...
bool ReadActorAngles(const ActorIdentity& identity, ActorAngles& angles) {
    void* storage = FindComponentStorage(identity.entityContext, tane::offsets::render::kActorRotationComponentHash);
    void* rotation = storage != nullptr
        ? ResolveComponentFromStorage(
            storage,
            identity.entityId,
            tane::offsets::render::kActorRotationComponentEntrySize)
        : nullptr;
    if (rotation == nullptr) {
        return false;
    }

    return ReadOffset(rotation, tane::offsets::render::kActorRotationPitchOffset, angles.pitch) &&
        ReadOffset(rotation, tane::offsets::render::kActorRotationYawOffset, angles.yaw) &&
        std::isfinite(angles.pitch) &&
        std::isfinite(angles.yaw);
}

Vec3 ForwardFromAngles(const ActorAngles& angles) {
    constexpr float kDegreesToRadians = 3.14159265358979323846f / 180.0f;
    const float pitch = angles.pitch * kDegreesToRadians;
    const float yaw = angles.yaw * kDegreesToRadians;
...
void* GetSelectionBoxMaterial() {
    ...
    __try {
        hashedStringCtor(hashedStringStorage.data(), "selection_box");
...
bool DrawTracerLines3D(
    ...
    const Color& color) {
    if (screenContext == nullptr || tessellator == nullptr || material == nullptr || targets.empty()) {
        return false;
    }
    ...
    __try {
        ...
        begin(tessellator, 0, kLineListPrimitive, static_cast<int>(targets.size() * 2), false);
        setColor(tessellator, &color);
        for (const Vec3& target : targets) {
            vertex(tessellator, start.x - origin.x, start.y - origin.y, start.z - origin.z);
            vertex(tessellator, target.x - origin.x, target.y - origin.y, target.z - origin.z);
        }
...
bool GetActorTracerTarget(
    const ActorIdentity& identity,
    const ComponentStorageCache& componentCache,
    Vec3& target) {
    Vec3 renderPosition{};
    ...
    if (ReadAabb(identity, componentCache, box)) {
        ...
        target.x = (box.lower.x + box.upper.x) * 0.5f;
        target.y = (box.lower.y + box.upper.y) * 0.5f;
        target.z = (box.lower.z + box.upper.z) * 0.5f;
        return std::isfinite(target.x) && std::isfinite(target.y) && std::isfinite(target.z);
    }

    if (hasRenderPosition || hasCurrentPosition) {
        target = hasRenderPosition ? renderPosition : currentPosition;
        target.y += 0.9f;
        return true;
    }
...
bool GetCameraTracerStart(const Vec3& origin, const ActorIdentity& identity, Vec3& start) {
    Vec3 forward{};
    if (!tane::camera::GetFreeLookCameraForward(forward.x, forward.y, forward.z)) {
        ActorAngles angles{};
        if (!ReadActorAngles(identity, angles)) {
            return false;
        }
        forward = ForwardFromAngles(angles);
    }

    constexpr float kNearPlaneLift = 0.45f;
    start.x = origin.x + forward.x * kNearPlaneLift;
    start.y = origin.y + forward.y * kNearPlaneLift;
    start.z = origin.z + forward.z * kNearPlaneLift;
    return std::isfinite(start.x) && std::isfinite(start.y) && std::isfinite(start.z);
}

void SaveTracerConfig() {
    wchar_t path[MAX_PATH]{};
    if (!BuildTracerConfigPath(path, MAX_PATH)) {
        return;
    }

    const Color color = LoadTracerColor();
    char json[256]{};
    std::snprintf(
        json,
        sizeof(json),
...
void EnsureTracerConfigLoaded() {
    EnsureRenderLevelRenderHookInstalled();
...
void SetTracerLineColor(float r, float g, float b, float a) {
    EnsureTracerConfigLoaded();
    StoreTracerColor(Color{r, g, b, a});
    SaveTracerConfig();
}
...
void RenderTracers3D(void* levelRenderer, void* screenContext, void* clientInstance) {
    ...
    if (clientInstance == nullptr) {
        clientInstance = g_lastClientInstance.load(std::memory_order_acquire);
    }
    ...
    void* material = GetSelectionBoxMaterial();
    if (material == nullptr) {
        return;
    }
    ...
    ActorIdentity localIdentity{};
    if (!ReadActorIdentity(localPlayer, localIdentity)) {
        return;
    }

    Vec3 start{};
    if (!GetCameraTracerStart(origin, localIdentity, start)) {
        return;
    }

    static std::vector<Vec3> targets;
    targets.clear();
    if (targets.capacity() < actors.size()) {
    ...
    for (void* actor : actors) {
        ...
        Vec3 target{};
        if (!GetActorTracerTarget(identity, componentCache, target)) {
            continue;
        }
    ...
    const Color color = LoadTracerColor();
    DrawTracerLines3D(screenContext, tessellator, material, origin, start, targets, color);
}
